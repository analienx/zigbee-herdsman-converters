...
function smtonoffDevice(overrides: {manufacturerName?: string; endpoints?: typeof sparseEndpoints; ieeeAddr?: string} = {}) {
    const device = mockDevice({
        ...
        ieeeAddr: overrides.ieeeAddr,
    });
    // mockDevice intentionally leaves newer Basic fields undefined; provide the
    // captured values required by the production fingerprint matcher.
...
describe("SMTONOFF ZXB3-125 endpoint fingerprint", () => {
    it.each(["0xa4c138278695ef31", "0xa4c1381647fb6fe1"])("%s selects SMTONOFF", async (ieeeAddr) => {
        const definition = await findByDevice(smtonoffDevice({ieeeAddr}));
